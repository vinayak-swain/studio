...
## Studio CLI (DVCS) Setup

DevNest comes with a powerful CLI tool for terminal-based version control with AI insights. Since this is a local development environment, you need to link the CLI package to use it globally.
...
- `studio init <repoId>`: Initialize a repository tracking in current folder.
- `studio status`: See local sync status and tracked files.
- `studio push "message"`: Push all local files to the cloud with AI code analysis.
- `studio pull`: Pull the latest state from the cloud.
- `studio clone <repoId>`: Clone a repository from DevNest.
